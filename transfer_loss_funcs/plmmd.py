...
from transfer_loss_funcs.adv import LambdaSheduler
import torch
import torch.nn as nn
import numpy as np


class PLMMDLoss(MMDLoss, LambdaSheduler):
    def __init__(self, num_class, kernel_type='rbf', kernel_mul=2.0, kernel_num=5, fix_sigma=None,
                 gamma=1.0, max_iter=1000, **kwargs):
        """
        Local MMD
        """
        super(PLMMDLoss, self).__init__(kernel_type, kernel_mul, kernel_num, fix_sigma, **kwargs)
        super(MMDLoss, self).__init__(gamma, max_iter, **kwargs)
        self.num_class = num_class
        # 自适应plmmd权重参数，务必将其加入params参与更新；
        self.class_coefficient = nn.Parameter(torch.ones([self.num_class, 1]) * (1 / self.num_class), requires_grad=True)
    ...
    def forward(self, source, target, source_label, target_logits):
        if self.kernel_type == 'linear':
            ...
            loss += torch.sum(weight_ss * SS + weight_tt * TT - weight_st * ST - weight_ts * TS)
            # Dynamic weighting
            lamb = self.lamb()
            self.step()
            loss = loss * lamb
    ...
    def cal_weight(self, source_label, target_logits):
        ...
        for i in range(self.num_class):  # (B, C)
            customer_coefficient = self.class_coefficient[i].cpu().detach().numpy()
            # adaptive coefficient added by ra opt for every class
            if i in set_s and i in set_t:
                s_tvec = source_label_onehot[:, i].reshape(batch_size, -1)  # (B, 1)
                t_tvec = target_logits[:, i].reshape(batch_size, -1)  # (B, 1)

                ss = customer_coefficient*np.dot(s_tvec, s_tvec.T)  # (B, B)
                weight_ss = weight_ss + ss
                tt = customer_coefficient*np.dot(t_tvec, t_tvec.T)
                weight_tt = weight_tt + tt
                st = customer_coefficient*np.dot(s_tvec, t_tvec.T)
                weight_st = weight_st + st  # why not calculate the ts?
                ts = customer_coefficient*np.dot(t_tvec, s_tvec.T)
