...
mat_data_dir = "/45TB/lyb/Dataset/For_the_strange_Benchmark/CWRU48K"
...
def get_keys_of_raw_data():
    for j in range(4):
        for num, i in enumerate(class_list):
            raw_data = loadmat(os.path.join(mat_data_dir, i+str(j)+".mat"))
            info = "[wc:{}][class:{}][keys:{}]".format(j, i, raw_data.keys())
            print(info)
...
def get_data_from_file(root: str, wc: str, direction=0, signal_size=1024):
    ...
    for num, i in enumerate(class_list):
        ...
        if "DE" in list(raw_data.keys())[3]:
            ...
            raw_data = raw_data[list(raw_data)[3]][:235520, :]
            # 驱动端数据
        elif "DE" in list(raw_data.keys())[4]:
            raw_data = raw_data[list(raw_data)[4]][:235520, :]
        else:
...
class CWRUBear48K1DaFFT(object):
    # num_class = 5
    # input_channel = 1
...
if __name__ == "__main__":
    ...
    # CWRUB = CWRUBear48K1DaFFT(mat_data_dir, "0-1")
    # CWRUB.data_prepare()
    get_keys_of_raw_data()
