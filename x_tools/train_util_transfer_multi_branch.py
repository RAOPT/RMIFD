...
class train_util_transfer_multi_branch:
    ...
    def get_model(self):
        return networks_transfer.transfernet1d_multi_branch(num_class=self.args.num_class,
                                                            basic_net=self.args.basic_net,
    ...
    def train(self):
        ...
        for e in range(self.start_epoch, self.args.n_epoch):
            ...
            if self.args.is_descend:
                transfer_loss_weight = 2 / (1 + math.exp(-10 * e / self.args.n_epoch)) -1
            else:
                transfer_loss_weight = self.args.transfer_loss_weight
            for n in range(len_of_loader):
                ...
                total_loss = cls_loss + transfer_loss_weight*transfer_loss
        ...
        line = name + ": Best accuracy {:.4f} got in epoch {} by {}".format(best_acc, best_epoch,
                                                                            self.args.basic_net) + "\n"
        print(line)
        # with open("Best_acc.txt", "a", encoding="utf-8") as f:
        #     f.write(line)
        with open(self.args.acc_preview, "a", encoding="utf-8") as f:
            f.write(line)
